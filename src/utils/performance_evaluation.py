...
def calc_loss_based_on_target_mean(label: str = "age", data_path: Path = AGE_SEX_BALANCED_10K_PATH):
    """Calculate the MSE loss between targets and their mean (training set) for a regression dataset."""
    data_split = DataSplitFile(data_path).load_data_splits_from_file()
    ...
    val_mean_tensor = torch.full_like(val_targets, train_mean.item())
    test_mean_tensor = torch.full_like(test_targets, train_mean.item())
    
    
    # Compute loss only with the train mean tensor
    loss_func = nn.MSELoss()
    train_loss = loss_func(train_targets, train_mean_tensor)
    val_loss = loss_func(val_targets, val_mean_tensor)
    test_loss = loss_func(test_targets, test_mean_tensor)
    ...
    val_results = val_metrics(val_targets, val_mean_tensor)
    test_results = test_metrics(test_targets, test_mean_tensor)
