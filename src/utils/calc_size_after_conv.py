...
class ConvCalculator:
    def __init__(
        self, kernel_size=5, stride=1, padding=1, pool_kernel_size=2, pool_stride=2
    ):
    ...
    def calculate_network_output_size(self, data_shape):
        """Calculate final output size after 4 convolutions and 2 maxpools."""
        # Conv1
        size = self._compute_conv_size(data_shape)
        # Maxpool1
    ...
    def test_file(self, subject_id: int, feature_map: FeatureMapType) -> None:
        ...
        data_shape = image_file.get_size()
        output_size = self.calculate_network_output_size(data_shape)
        print(f"Testing file: {image_file.file_path}")
        print(f"File parameters: {feature_map.modality_label}, {feature_map.label}")
        print(f"Input size feature map: {data_shape}")
        print(f"Output size feature map (for FC layer): {output_size}")

    def _compute_conv_size(self, data_shape):
        """Calculate output size after convolution for arbitrary dimensions."""
        return tuple(
            (dim - self.kernel_size + 2 * self.padding) // self.stride + 1
            for dim in data_shape
        )

    def _compute_maxpool_size(self, data_shape):
        """Calculate output size after maxpooling for arbitrary dimensions."""
        # remove batch dimension
        return tuple(
            (dim - self.pool_kernel_size) // self.pool_stride + 1 for dim in data_shape
        )
